...
static int __init led_lkm_init(void)
{
    ...
    gpriv = dev_kzalloc(dev, sizeof(struct ledLkmCtx), GFP_KERNEL);
    gpriv->dev = dev;
    gpriv->powered = ATOMIC_INIT(0);
    return 0;
}
...
static ssize_t read_led_lkm(struct file *filp, char __user *ubuf, szie_t count, loff_t *oof) {
    int ret, powered;
    ret = -EFAULT;
